export const defaultTheme = {
  id: "defaultTheme",
  body: '#EFEFEF',
  text: '#616161',
  border: '2px solid #FF3B3F',
  emphasisBorder: '4px solid #FF3B3F',
  fill: '#CAEBF2',
  headerText: '#616161',
  dropdownText:'#616161',
  inputbackground: '#CAEBF2',
  button: {
    text: '#EFEFEF',
    background: '#FF3B3F'
  }
}

export const darkTheme = {
  id: "darkTheme",
  body: '#031424',
  text: '#8EAEBD',
  border: '2px solid #CF6766',
  emphasisBorder: '4px solid #CF6766',
  fill: '#30415D',
  headerText: '#CF6766',
  dropdownText: '#031424',
  inputbackground: '#8EAEBD',
  button: {
    text: '#EFEFEF',
    background: '#CF6766'
  }
}

export const calmTheme = {
  id: "calmTheme",
  body: '#D5D5D5',
  text: '#616161',
  border: '2px solid #6D7993',
  emphasisBorder: '4px solid #6D7993',
  fill: '#96858F',
  headerText: '#616161',
  dropdownText: '#616161',
  inputbackground: '#9099A2',
  button: {
    text: '#EFEFEF',
    background: '#96858F'
  }
}

export const warmTheme = {
  id: "warmTheme",
  body: '#DCC7AA',
  text: '#6B7A8F',
  border: '2px solid #F7882F',
  emphasisBorder: '4px solid #F7882F',
  fill: '#6B7A8F',
  headerText: '#DCC7AA',
  dropdownText: '#6B7A8F',
  inputbackground: '#F7C331',
  button: {
    text: '#F7C331',
    background: '#F7882F'
  }
}

export const hiContrastTheme = {
  id: "hiContrastTheme",
  body: '#000814',
  text: '#FFD60A',
  border: '2px solid #FFD60A',
  emphasisBorder: '4px solid #FFD60A',
  fill: '#003566',
  headerText: '#FFD60A',
  dropdownText: '#000814',
  inputbackground: '#FFD60A',
  button: {
    text: '#FFD60A',
    background: '#003566'
  }
}